...
public class CylindermasterDao extends BaseCylindermasterDao
{
	...
	public List<CylindermasterBean> getCylinders(){  
		jdbcTemplate = custom.getJdbcTemplate();
		 ...
		 String sql =  "SELECT co.companyname , c. *,cs.name as cylinderstatus,i.name As sizeName,DATE_FORMAT(c.expirydate,'%d-%M-%Y') AS expirtdate1 ,  CASE WHEN c.status IN ('0') THEN 'Deactive' WHEN c.status in ('1') THEN 'Active'  ELSE '-----' END as cylendersstatus   FROM companymaster co,cylindermaster c,items i,cylinderstatus cs where c.size=i.id and cs.id=c.cylinderstatus and co.id=c.ownercompany order by c.id desc";
